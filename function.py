...
def calc_mean_std(feat, eps=1e-5):
    ...
    size = feat.size()
    assert (len(size) == 4)
    N, C = size[:2]  # N是批量大小，C是通道数
    feat_var = feat.view(N, C, -1).var(dim=2) + eps  # 计算标准差
    feat_std = feat_var.sqrt().view(N, C, 1, 1)  # 计算方差
...
def mean_variance_norm(feat):
    ...
    return normalized_feat  # 均值方差归一化公式，x-均值除以标准差
...
def cosine_dismat(A, B):
    ...
    A_norm = torch.sqrt((A ** 2).sum(1))  # A每一行平方求和开根号
